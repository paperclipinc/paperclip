 ...
 * (authenticated) mode, only once a single company exists, only if the viewer
 * OWNS that company (an invited member landing on a shared company must never
 * see the rename-your-company wizard), and only if it has not already been
 * completed for that company.
 *
 * `viewerRole` is the current user's membership role for the company
 * (access.currentUserRole from GET /companies/:id/members). It is intentionally
 * required-but-nullable: when the role is still loading (`undefined`) or absent
 * (`null`) we hold the wizard back rather than risk showing it to an invited
 * member. Only an explicit "owner" opens the rename flow.
 */
export function shouldOpenCloudOnboarding(params: {
  ...
  const { deploymentMode, companyId, viewerRole } = params;
  if (deploymentMode !== "authenticated") return false;
  if (!companyId) return false;
  // Owner-only: invited members (admin/operator/viewer or no role yet) must
  // never be shown the rename-your-company wizard on a shared company.
