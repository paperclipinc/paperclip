...
// Event `type` values emitted by the OpenCode JSONL stream. Managed cloud
// agents run through the process wrapper (adapterType "process") but the
// underlying harness still emits this exact shape, so we detect it and reuse
// the existing rich renderer instead of dumping raw JSON.
 ...
 *
 * The process wrapper interleaves a few plain bootstrap lines (e.g.
 * `[paperclip] Syncing skills ...`) before the harness's structured JSONL
 * stream begins. Once we detect an OpenCode-shaped JSONL line we lock on and
