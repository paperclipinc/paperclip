...
export async function createApp(
  ...
  //
  // Fork delta: self-heal a bundled kubernetes plugin stuck in a non-ready
  // status. ensureBundledPlugins skips any plugin already present in a
  // non-uninstalled state, so a record stuck in 'error' (e.g. an earlier boot
  // saw a missing manifest before the image shipped it) would leave the
  // kubernetes provider dead and agents unable to acquire leases. Drive it
  // back to 'ready' here so loadAll() re-activates it. Fully fail-safe.
  const healStuckBundledKubernetesPlugin = async (): Promise<void> => {
    ...
    try {
      ...
      } else if (action === "self-heal-blocked-bundle-missing") {
        logger.warn(
      ...
      logger.error(
        { err, pluginKey: KUBERNETES_PLUGIN_KEY },
  ...
  void ensureBundledPlugins(
    ...
    .then(() => healStuckBundledKubernetesPlugin())
    .then(() => loader.loadAll())
    .then((result) => {
    if (!result) return;
