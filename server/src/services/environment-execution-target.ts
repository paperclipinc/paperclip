        ...
        ? {
            ...
            execute: async (commandInput) => {
              ...
              try {
                ...
                // through only the suffix the provider did NOT already stream
                // through onLog. The streamed chunks usually form an in-order
                // prefix of the final result, so the remaining output is the
                // final text past the delivered text. When the provider streamed
                // nothing, the whole output is the suffix. When it streamed the
                // complete output, the suffix is empty and nothing repeats. When
                // it streamed a prefix and then fell back to a poll whose buffer
