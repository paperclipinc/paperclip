...
describeEmbeddedPostgres("environmentRuntimeService", () => {
  ...
  async function seedReusablePluginSandboxLease(
    // Two call shapes coexist after the merge: upstream passes the adapter type
    // positionally, the fork's cases pass `{ scopedAdapterType }`.
    seedInput: string | null | { scopedAdapterType?: string | null } = null,
  ) {
    const adapterType =
      typeof seedInput === "string" || seedInput === null
        ? seedInput
        : (seedInput.scopedAdapterType ?? null);
  ...
  it("resumes the same provider lease on the next per-turn run", async () => {
    ...
    const first = await runtimeWithPlugin.acquireRunLease({
      ...
      // The fork requires a POSITIVE adapter-type match to reuse a plugin
      // sandbox lease (never null-on-either-side), so the run has to name one.
      adapterType: "codex_local",
  ...
  it("reports the plugin cleanup worker not ready while its worker is down, then ready after it recovers", async () => {
    ...
    const workerManager = {
      // Upstream reads the worker's advertised methods before every lease RPC.
      getWorker: vi.fn(() => ({
        supportedMethods: [
  ...
  it("preserves the exact lease when plugin-backed sandbox resume throws", async () => {
    ...
    await expect(runtimeWithPlugin.acquireRunLease({
      ...
      agentId,
      // The fork requires a POSITIVE adapter-type match to reuse a plugin
      // sandbox lease (never null-on-either-side), so the run names one.
      adapterType: "codex_local",
      heartbeatRunId: runId,
      persistedExecutionWorkspace: {
        id: executionWorkspaceId,
  ...
  it("resumes a legacy secret-blind reusable sandbox lease via the runtime fallback when the environment has no secret refs", async () => {
    ...
    //
    // The lease is scoped to a concrete adapter type (matched by the run's
    // adapterType) so it satisfies the plugin-backed positive-match rule
    // (reusableSandboxLeaseScopeMatches requires storedAdapterType and the run's
    // adapterType to both be set and equal for a plugin-backed lease). A
    // null-scope plugin lease is deliberately NOT reused; that case is covered by
    // "does not reuse a plugin-backed sandbox lease when both the stored scope
    // and the run's adapter type are null". This test isolates the secret-blind
    // legacy runtime-fingerprint fallback, which sits behind that adapter gate.
    const scopedAdapterType = "claude_local";
    const { pluginId, companyId, agentId, environment, runId, executionWorkspaceId } =
      await seedReusablePluginSandboxLease({ scopedAdapterType });
    const workerManager = {
      // Upstream reads the worker's advertised methods before every lease RPC.
      getWorker: vi.fn(() => ({
        supportedMethods: [
