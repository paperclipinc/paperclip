...
export async function testEnvironment(
  ...
  if (canRunProbe) {
    if (!claudeCommandLooksLike(command, "claude")) {
      ...
      if (probe.timedOut) {
      ...
      } else if (loginMeta.credentialRejected) {
        // A credential was actually presented and the provider rejected it.
        // This is a harder failure than a simple "please log in" prompt: the
        // user pasted a key and it is invalid, so treat it as an error-level
        // gate rather than a warn-level nudge.
        logSandboxProbeDiagnostic(
          "Claude CLI hello probe reported credential rejected",
          "credential_rejected",
        );
        checks.push({
          code: "claude_hello_probe_credential_rejected",
          level: "error",
          message: "Claude rejected the configured credential.",
          hint: "Replace the invalid credential, or remove it so the stored login can be used.",
          authFailure: true,
        });
        ...
        // An auth rejection that was not caught by the credentialRejected gate
        // above (e.g. an unrecognized wording that isClaudeInvalidCredentialError
        // still matches) should surface as authFailure on the failed check so the
        // UI gate closes.
        const authFailure = loginMeta.credentialRejected ||
          isClaudeInvalidCredentialError({ parsed, stdout: probe.stdout, stderr: probe.stderr });
        checks.push(
          usageLimited
            ? {
