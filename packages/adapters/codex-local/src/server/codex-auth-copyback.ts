...
export async function copyBackCodexAuth(input: CopyBackCodexAuthInput): Promise<CopyBackCodexAuthOutcome> {
  ...
  // seeded from. See the ENOENT handling below.
  let hostOutcome: CopyBackCodexAuthOutcome;
  try {
    hostOutcome = await withDirectoryMergeLock(hostDir, async () => {
