...
export async function assertCodexCredentialsLaunchable(input: {
  ...
  // A Codex `auth.json` the user minted with `codex login` on their own
  // machine (the ChatGPT-plan credential path). Threaded through to
  // `evaluateCodexCredentialReadiness` so a configured plan credential counts
  // as ready even when the host has no managed auth.json of its own.
  ...
  if (targetIsSandbox) {
    ...
    throw new Error(
      `no Codex credentials provisioned for managed home "${input.effectiveCodexHome}" ` +
        ...
        `supply a ChatGPT-plan credential (CODEX_AUTH_JSON) minted with \`codex login\` on your ` +
        `own machine, or sign in to Codex on the host with a ChatGPT subscription.`,
    );
  }
