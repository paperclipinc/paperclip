...
describe("sandbox callback bridge", () => {
  ...
  it("treats a transient listJsonFiles failure as recoverable and keeps processing", async () => {
    ...
    // In-memory queue: listJsonFiles throws twice (a transient exec/WebSocket
    // blip) BEFORE the request file ever appears, then recovers. A worker that
    // permanently settled on the first throw would never see "req-1".
    let listCalls = 0;
    ...
    const worker = await startSandboxCallbackBridgeWorker({
      client: {
        makeDir: async () => {},
        listJsonFiles: async (dir: string) => {
          if (dir === directories.requestsDir) {
            listCalls += 1;
            if (listCalls <= 2) {
              throw new Error(
                "list /remote/queue/requests failed with exit code 1: transient channel blip",
              );
            }
            // After the blips recover, surface the queued request once.
        ...
        readTextFile: async (filePath: string) => {
          ...
          if (raw == null) throw new Error(`missing ${filePath}`);
          return raw;
        },
        writeTextFile: async () => {},
        rename: async () => {},
        remove: async (filePath: string) => {
          files.delete(path.posix.basename(filePath));
        },
      },
      queueDir,
      pollIntervalMs: 5,
    ...
    // Make the request visible only after the worker has already hit (and
    // survived) the transient listing failures.
    while (listCalls < 2) {
      await new Promise((resolve) => setTimeout(resolve, 5));
    }
    files.set("req-1.json", JSON.stringify({ id: "req-1", method: "GET", path: "/api/agents/me" }));
    ...
    expect(processed).toEqual(["req-1"]);
  });
